	...
	public function Query()
	{
		try
		{
			...
			if ($page != '')
			{
				...
				$imoveis = $this->Phreezer->Query('Imovel',$criteria)->GetDataPage($page, $pagesize);
				$output->rows = $imoveis->ToObjectArray(true,$this->SimpleObjectParams());
				$output->totalResults = $imoveis->TotalResults;
				$output->totalPages = $imoveis->TotalPages;
				$output->pageSize = $imoveis->PageSize;
				$output->currentPage = $imoveis->CurrentPage;
			}
			else
			{
				// return all results
				$imoveis = $this->Phreezer->Query('Imovel',$criteria);
				$output->rows = $imoveis->ToObjectArray(true, $this->SimpleObjectParams());
