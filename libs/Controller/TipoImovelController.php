	...
	public function Query()
	{
		try
		{
			...
			if ($page != '')
			{
				...
				$tipoimoveis = $this->Phreezer->Query('TipoImovel',$criteria)->GetDataPage($page, $pagesize);
				$output->rows = $tipoimoveis->ToObjectArray(true,$this->SimpleObjectParams());
				$output->totalResults = $tipoimoveis->TotalResults;
				$output->totalPages = $tipoimoveis->TotalPages;
				$output->pageSize = $tipoimoveis->PageSize;
				$output->currentPage = $tipoimoveis->CurrentPage;
			}
			else
			{
				// return all results
				$tipoimoveis = $this->Phreezer->Query('TipoImovel',$criteria);
				$output->rows = $tipoimoveis->ToObjectArray(true, $this->SimpleObjectParams());
